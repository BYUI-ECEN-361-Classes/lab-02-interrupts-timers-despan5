...
void got_start()
{
    ...
    */
    ...
    // Step 1
    MultiFunctionShield_Display(0);  // Display Waiting ----

    // Step 2
    HAL_Delay(rand_millisec);  // Wait for a random amount of time

    // Step 3
    MultiFunctionShield_Display("GO");  // Turn on all the 7-Seg lights as "GO"

    // Step 4
    HAL_TIM_Base_Start_IT(&htim3);  // Start the Reaction timer
...
void got_stop()
{
     ...
     */
    // 1.) Stop the timer
    HAL_TIM_Base_Stop_IT(&htim3);  // Stop the random timer
